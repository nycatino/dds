...
//Dentro del método findAll, se llama al método findAll() del repository, que se encarga de acceder a la base de datos y recuperar todos los registros de la entidad Usuario. El resultado, que es una lista de usuarios, se devuelve al llamador de este método.
	...
	public Tema updateTema(Tema tema) {
	    ...
	    // tema.setId(id); // Asegúrate de establecer el ID
	     return repository.save(tema); // Guarda el curso actualizado
	}
    ...
    public void deleteById(Long id) {
    	 List<Curso> cursos = cursoRepository.findByTemaId(id);

         // Actualizar los cursos (puedes establecer el docente a null o eliminar los cursos)
         for (Curso curso : cursos) {
             cursoRepository.delete(curso); // Guardar cambios en el curso
         }
    ...
    public Tema findTemaById(Long id) {
		return repository.findTemaById(id);
	}
